...
def create_pill(conn, pill):
    ...
    sql = ''' INSERT INTO pills(id,name,generic_name,drug_class)
              VALUES(?,?,?,?) '''
    cur = conn.cursor()
...
def bulk_pills(conn):
    ...
    cur = conn.cursor()
    cur.executemany("""
        INSERT OR IGNORE INTO pills VALUES (?, ?, ?, ?)""", pills)

def main():
    database = "/home/ruifgmonteiro/Desktop/FRESH/webservice/pills.db"
